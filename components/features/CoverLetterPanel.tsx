      ...
      {options.map((o) => (
        <button
          ...
          className={`flex-1 rounded-xl border text-left px-3 py-2 transition-all text-xs ${
            value === o.value
              ? `border-${color}-400 bg-${color}-50 ring-1 ring-${color}-400`
              : "border-slate-200 hover:border-slate-300 bg-white"
          }`}
        >
          <span className={`font-semibold block ${value === o.value ? `text-${color}-700` : "text-slate-700"}`}>
            {o.label}
          </span>
      ...
      <AnimatePresence>
        {isPromptOnly && promptText && !isLoading && (
          <motion.div key="cl-prompt" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="p-3 text-xs font-mono text-slate-500 max-h-64 overflow-y-auto whitespace-pre-wrap custom-scrollbar bg-white">
              {promptText}
            </div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {isStreaming && (
          <motion.div key="cl-stream" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="rounded-xl bg-indigo-50 border border-indigo-200 overflow-hidden">
            <div className="px-4 py-2.5 bg-indigo-100/60 flex items-center gap-2 border-b border-indigo-200">
        ...
        {hasResult && !isLoading && (
          <motion.div key="cl-result" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <textarea
              ...
              className="w-full min-h-[320px] max-h-[500px] text-xs text-slate-700 leading-relaxed border border-slate-200 rounded-xl p-4 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 resize-y custom-scrollbar bg-white"
            />
            <div className="flex gap-2">
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button size="sm" variant="outline"
                    ...
                    className="h-7 px-3 text-xs border-slate-200 text-slate-600 hover:border-indigo-400 hover:text-indigo-600 hover:bg-indigo-50">
