...
function ZoneCard({ accent, title, children, className = "" }: {
  ...
  return (
    <div className={`rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden flex flex-col ${className}`}>
...
export default function Home() {
  ...
  const getModelOptions = useCallback((p = provider) => {
    switch (p) {
      ...
      case "ollama": return ollamaModels.length > 0 ? ollamaModels : ["llama3", "mistral", "gemma3:4b"];
  ...
  const refreshOllamaModelsManually = async () => {
    setIsRefreshingModels(true);
    const models = await fetchOllamaModels();
    setOllamaModels(models);
    setIsRefreshingModels(false);
    if (models.length === 0) toast.warning("No Ollama models found. Is Ollama running?");
    else toast.success("Ollama models refreshed");
  };
  ...
  useEffect(() => {
    ...
    const initializeProvider = async () => {
      setIsRefreshingModels(true);
      try {
        const [configRes, ollamaModelList] = await Promise.all([
        ...
        setConfiguredProviders(configRes);
        setOllamaModels(ollamaModelList);

        // Determine default provider
        let defaultProvider: typeof provider = "prompt-only";
        ...
        else if (ollamaModelList.length > 0) defaultProvider = "ollama";

        setProvider(defaultProvider);
        
        // Initialize default model for the selected provider
        const models = defaultProvider === "ollama" ? ollamaModelList : getModelOptions(defaultProvider);
        if (models.length > 0) {
  ...
  const handleProviderChange = (v: typeof provider) => {
    setProvider(v);
    
    // Try to restore saved API key for this provider
    const savedKey = typeof window !== "undefined" ? localStorage.getItem(`rh_apikey_${v}`) : null;
    setApiKey(savedKey || "");
    
    setIsCustomModel(false);
    // Try to restore a saved custom model for this provider
    const saved = typeof window !== "undefined" ? localStorage.getItem(`rh_model_${v}`) : null;
    const models = getModelOptions(v);
    if (saved && !models.includes(saved)) {
      // Saved custom model — restore it
      setSelectedModel(saved);
      setCustomModelInput(saved);
    ...
    if (v === "ollama" && ollamaModels.length === 0) refreshOllamaModelsManually();
  };
      ...
      for (const strategy of strategies) {
        ...
        try {
          ...
          if (parsed && (parsed.personalInfo || parsed.workExperience || parsed.summary)) {
            ...
            setOriginalResumeData((prev) => prev);
  ...
  const handleGenerate = async () => {
    ...
    if (!originalResumeData) setOriginalResumeData(resumeData);
    if (provider !== "prompt-only") {
      setCompletion("");
                        ...
                        onClick={async () => {
                          try {
                            ...
                            if (text.trim()) { 
                              const val = text.trim();
                              setApiKey(val); 
                              if (typeof window !== "undefined") localStorage.setItem(`rh_apikey_${provider}`, val);
