  ...
  const generateStarFlashcard = async (questionObj: InterviewQuestion) => {
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        // Scroll to the response on mobile
    ...
    <div className="max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-8 min-h-screen space-y-8">
      ...
      {!isReady && (
        <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-xl p-6 text-center max-w-2xl mx-auto mt-12">
          ...
          <p className="text-sm text-amber-700 dark:text-amber-500/70 mb-6">Head over to the Optimizer, load your resume, paste a job description, and hit Save Job or Analyze to activate this tool.</p>
